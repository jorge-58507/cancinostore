...
class measureproductController extends Controller
{
    public function save(Request $request, $product_slug){
        ...
        //ANSWER
        $rs_measure = cs_product::select('cs_measures.tx_measure_value','rel_measure_products.tx_measure_product_relation','rel_measure_products.ai_measure_product_id')->join('rel_measure_products','rel_measure_products.measure_product_ai_product_id','=','cs_products.ai_product_id')
        ->join('cs_measures','rel_measure_products.measure_product_ai_measure_id','=','cs_measures.ai_measure_id')->where('tx_product_slug',$product_slug)->where('tx_measure_status',1)->get();
    ...
    public function delete($rel_id){
        ...
        $rs->delete();
                ...
                //ANSWER
        $rs_measure = cs_product::select('cs_measures.tx_measure_value','rel_measure_products.tx_measure_product_relation','rel_measure_products.ai_measure_product_id')->join('rel_measure_products','rel_measure_products.measure_product_ai_product_id','=','cs_products.ai_product_id')
        ->join('cs_measures','rel_measure_products.measure_product_ai_measure_id','=','cs_measures.ai_measure_id')->where('ai_product_id',$rs['measure_product_ai_product_id'])->where('tx_measure_status',1)->get();
